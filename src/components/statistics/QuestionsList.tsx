  ...
  return (
    ...
    <TableHeader className="bg-gray-100 dark:bg-gray-800">
      <TableRow>
        ...
        <TableHead className="p-2 border border-gray-300 dark:border-gray-700">Question & Correct Answer</TableHead>
        <TableHead className="p-2 border border-gray-300 dark:border-gray-700">Your Answer</TableHead>
        {questions[0].questionType === "open_ended" && (
          <TableHead className="w-[10px] p-2 border border-gray-300 dark:border-gray-700 text-right">Accuracy</TableHead>
        )}
      </TableRow>
    </TableHeader>
      ...
      {questions.map(
        (
          ...
          <TableRow key={index} className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'}`}>
            ...
            {questions[0].questionType === "open_ended" ? (
              <TableCell className="font-semibold p-2 border border-gray-300 dark:border-gray-700">{userAnswer}</TableCell>
            ) : (
              <TableCell className={`p-2 border border-gray-300 dark:border-gray-700 font-semibold ${isCorrect ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                {userAnswer}
              </TableCell>
            )}
            {percentageCorrect && (
